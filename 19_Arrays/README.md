...
**An array is a linear **data structure** in Java that stores multiple values of the same data type in contiguous memory locations. Arrays allow you to access data efficiently using an index.**
...
| **Type Safe**        | Java arrays are type-safe — only correct data types can be stored.         |
| **Array Traversing** | Traversing means looping from index `0` to `length - 1` to access elements.|
| **Common Error**     | Accessing an invalid index causes `ArrayIndexOutOfBoundsException`.     |
